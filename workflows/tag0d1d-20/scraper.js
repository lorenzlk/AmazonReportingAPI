/**
 * Amazon Associates Scraper - tag0d1d-20
 * 
 * This workflow scrapes data for Store ID: tag0d1d-20
 * Tracking IDs: twsmm-20, stylcasterm-20, defpenm-20, swimworldm-20, britcom03-20, on3m-20
 * 
 * SETUP:
...
const STORE_ID = 'tag0d1d-20';
const TRACKING_IDS = ['twsmm-20', 'stylcasterm-20', 'defpenm-20', 'swimworldm-20', 'britcom03-20', 'on3m-20'];
...
export default defineComponent({
  ...
  async run({ steps, $ }) {
    // Use default date range (Last 30 Days) - no date picker manipulation
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const dateStr = today.toISOString().split('T')[0]; // Use today's date as label
    
    console.log(`🚀 Starting scraper for Store ID: ${STORE_ID}`);
    console.log(`📅 Using default date range (Last 30 Days) - data labeled as ${dateStr}`);
